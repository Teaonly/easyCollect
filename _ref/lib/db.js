...
db.loadDatabase = function(dbPath, cb) {
  ...
  db._index.sources['gist'] = "gist";
  db._index.sources['weibo'] = "新浪微博";
  //db._index.sources['pan.baidu'] = "百度云盘";
  db._index.sources['twitter'] = "Twitter";
  db._index.sources['github'] = "Github";
};
...
db.getDataByStar = function(cb) {
  db._data.find({star:'true'})
    .sort({'seconds':-1})
    .exec(function(err, docs) {
      assert(err === null);
      cb(docs);
...
db.getDataByAndTags = function(tags, cb) {
  var sql = {};
  sql.$and = [];
  for(var i = 0; i < tags.length; i++) {
    sql.$and.push( {'tags':tags[i]} );
  }
...
db.getDataByOrTags = function(tags, cb) {
  var sql = {};
  sql.$or = [];
  for(var i = 0; i < tags.length; i++) {
    sql.$or.push( {'tags':tags[i]} );
  }
...
db.updateStar = function(post) {
  db._data.update({'index':post.index}, { $set: {"star": post.star} }, {}, function(){});
};
...
db.removeCollect = function(collect, cb) {
